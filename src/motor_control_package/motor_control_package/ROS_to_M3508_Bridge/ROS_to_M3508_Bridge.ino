...
void update_output_message() {
  ...
  for (int i = 0; i < 4; i++) {  // Only update for motor IDs 1-4
    output_message += String(i + 1) + ",";  // Motor ID
    output_message += String(motors_speed[i]) + ",";  // Motor speed
    output_message += String(motors_angle[i]) + ",";  // Motor angle
    output_message += String(motors_torque[i]);  // Motor torque

    if (i < 3) {
      output_message += ";";  // Separate motor data with a semicolon
    }
  }
}
