...
class OmniPID(Node):
    ...
    def __init__(self):
        ...
        self.pid_controllers = [PIDController(0.35, 0.0, 0.00) for _ in range(4)]  # Adjusted PID controllers for each motor
    ...
    def control_motors(self):
        ...
        self.get_logger().error(f'motor_speeds: {self.motor_speeds}')
        # self.get_logger().info(f'target_speeds: {target_speeds}')
        # self.get_logger().info(f'control_signals: {control_signals}')
